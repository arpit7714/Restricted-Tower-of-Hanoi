 ...
 * @author devendra
 */
public class lab7 extends JFrame implements Runnable ,ActionListener{ 
    ...
    public lab7() {
       /* tm=new Timer(100,new ActionListener(){
        ...
        String number=JOptionPane.showInputDialog(null,"enter the number of disks");
     ...
     //disk[0] large size
         ...
         //	110+i*10+n*5
          disk[i]=new Rectangle(250-((250-n)-i*20)/2,475-i*25,(250-n)-i*20,25);
...
public void towerofhanoi(int number,int source, int intermediate, int dest)
{
       ...
       if (number==-1){
        ...
        //   inst+=1;
             /*int temp=0;
              int finalv=2*(disk[num1].y)/3;
            while(temp<=finalv){*/         
            disk[num1].setLocation(disk[num1].x,disk[num1].y-2*(disk[num1].y)/3);
            repaint();
            call(fps); 
            movecount.setText("Moves count="+ (++count));
            moveddisk.setText("Current mov --- Disk "+(num1+1)+ "   moving from"+(char)(source+87)+" --> "+(char)(intermediate+87)); 
            disk[num1].setLocation(disk[num1].x+(peg[intermediate-1].x-peg[source-1].x),disk[num1].y);
            repaint();
            call(fps);
  ...
  //this is the orerride method present in the jframe
   public void paint(Graphics g){
       ...
       //drawing disks
      for(int i=0;i<n;i++)
         {  //Color randomColor1 = new Color((float)Math.random(), (float)Math.random(), (float)Math.random()); 
          g.setColor(Color.green);
           g.fillRoundRect(disk[i].x,disk[i].y,disk[i].width,disk[i].height,20,20);
           g.setColor(Color.BLACK);
           g.drawRoundRect(disk[i].x,disk[i].y,disk[i].width,disk[i].height,20,20);
           g.setColor(Color.black);
           g.drawString(""+(i+1), disk[i].x+(disk[i].height/2),disk[i].y+13);
